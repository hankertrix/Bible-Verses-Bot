...
full_chapter_num_regex = re.compile(r"\b(?!chapter|verse|to\b)\w+? ?\d[\d, \-]*(?! :|:)\b ?(?:rcu17[ts]s)?[A-Za-z\-]*|gr?e?e?k esthe?r? ?\d[\d, \-]*(?! :|:)\b ?(?:rcu17[ts]s)?[A-Za-z\-]*|addi?t?i?o?n?s? to esthe?r? ?\d[\d, \-]*(?! :|:)\b ?(?:rcu17[ts]s)?[A-Za-z\-]*|(?:lett?e?r?|ep) ?(?:of)? ?jere?m?i?a?h? ?\d[\d, \-]*(?! :|:)\b ?(?:rcu17[ts]s)?[A-Za-z\-]*")

# Regex for finding the full chapter with the word "chapter"
full_chapter_chapt_regex = re.compile(r"\b[1234]? ?\w+? chapters? \d[\d, \-]*\b(?! verse|verse) ?(?:rcu17[ts]s)?[A-Za-z\-]*|gr?e?e?k esthe?r? chapters? \d[\d, \-]*\b(?! verse|verse) ?(?:rcu17[ts]s)?[A-Za-z\-]*|addi?t?i?o?n?s? to esthe?r? chapters? \d[\d, \-]*\b(?! verse|verse) ?(?:rcu17[ts]s)?[A-Za-z\-]*|(?:lett?e?r?|ep) ?(?:of)? ?jere?m?i?a?h? chapters? \d[\d, \-]*\b(?! verse|verse) ?(?:rcu17[ts]s)?[A-Za-z\-]*")
