...
full_chapter_num_regex = re.compile(r'\b(?!chapter|verse|to\b)[A-Za-z]+? \d\d?\d?[\d, -]*(?! :|:)\b ?(?:rcu17ss|rcu17ts)?[A-Za-z-]*|gr?e?e?k esthe?r? \d\d?\d?[\d, -]*(?! :|:)\b ?(?:rcu17ss|rcu17ts)?[A-Za-z-]*|addi?t?i?o?n?s? to esthe?r? \d\d?\d?[\d, -]*(?! :|:)\b ?(?:rcu17ss|rcu17ts)?[A-Za-z-]*|(?:lett?e?r?|ep) ?(?:of)? ?jere?m?i?a?h? \d\d?\d?[\d, -]*(?! :|:)\b ?(?:rcu17ss|rcu17ts)?[A-Za-z-]*')

# Regex for finding the full chapter with the word "chapter"
full_chapter_chapt_regex = re.compile(r'\b[1234]? ?[A-Za-z]* chapters? \d\d?\d?[\d, -]*\b(?! verse|verse) ?(?:rcu17ss|rcu17ts)?[A-Za-z-]*|gr?e?e?k esthe?r? chapters? \d\d?\d?[\d, -]*\b(?! verse|verse) ?(?:rcu17ss|rcu17ts)?[A-Za-z-]*|addi?t?i?o?n?s? to esthe?r? chapters? \d\d?\d?[\d, -]*\b(?! verse|verse) ?(?:rcu17ss|rcu17ts)?[A-Za-z-]*|(?:lett?e?r?|ep) ?(?:of)? ?jere?m?i?a?h? chapters? \d\d?\d?[\d, -]*\b(?! verse|verse) ?(?:rcu17ss|rcu17ts)?[A-Za-z-]*')
